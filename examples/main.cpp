...
void hanging_net()
{
	...
	*/
	...
	auto [V, T] = nexdyndiff::generate_triangle_grid({ 0.0, 0.0 }, { d, d }, { n, n });
	auto E = nexdyndiff::find_edges_from_simplices(T, V.size());

	// Segment Net
	auto H = simulation.presets->deformables->add_line("segments", V, E, nexdyndiff::Line::Params::Elastic_Rubberband());
	
	// BC
	auto bc1 = simulation.deformables->prescribed_positions->add_outside_aabb(H.point_set, { 0.0, 0.0, 0.0 }, { d - 0.001, d - 0.001, d - 0.001 }, nexdyndiff::EnergyPrescribedPositions::Params());
...
void hanging_cloth()
{
	...
	*/
	...
	const double d = 1.0;
	const double hd = d/2.0;
	auto [V, T, H] = simulation.presets->deformables->add_surface_grid("cloth", { d, d }, { n, n }, nexdyndiff::Surface::Params::Cotton_Fabric());

	// BC
	auto bc1 = simulation.deformables->prescribed_positions->add_inside_aabb(H.point_set, { hd, hd, 0.0 }, { 0.001, 0.001, 0.001 }, nexdyndiff::EnergyPrescribedPositions::Params());
	auto bc2 = simulation.deformables->prescribed_positions->add_inside_aabb(H.point_set, { -hd, hd, 0.0 }, { 0.001, 0.001, 0.001 }, nexdyndiff::EnergyPrescribedPositions::Params());
...
void hanging_deformable_box()
{
	...
	*/
	...
	const double d = 0.5;
	const double hd = d/2.0;
	auto [V, T, H] = simulation.presets->deformables->add_volume_grid("box", { d, d, d }, {n, n, n}, nexdyndiff::Volume::Params::Soft_Rubber());
	
	// BC
	auto bc1 = simulation.deformables->prescribed_positions->add_inside_aabb(H.point_set, { hd, hd, hd }, { 0.001, 0.001, 0.001 }, nexdyndiff::EnergyPrescribedPositions::Params().set_stiffness(1e7));
	auto bc2 = simulation.deformables->prescribed_positions->add_inside_aabb(H.point_set, { -hd, hd, hd }, { 0.001, 0.001, 0.001 }, nexdyndiff::EnergyPrescribedPositions::Params().set_stiffness(1e7));
...
void attachments()
{
	...
	*/
	...
	auto params = nexdyndiff::Surface::Params::Cotton_Fabric();
	...
	const nexdyndiff::Mesh<3> box_mesh = nexdyndiff::make_box({ bs, bs, bs });
	...
	auto att_cloth = simulation.interactions->attachments->add_by_distance(H2.point_set, H1.point_set, H2.point_set.all(), T1, 2.0*gap, 
		nexdyndiff::EnergyAttachments::Params().set_tolerance(0.01)
	);
	...
	// BC
	auto bc1 = simulation.deformables->prescribed_positions->add_inside_aabb(H1.point_set, { -hd, -hd, 0.0 }, { 0.001, 0.001, 0.001 }, nexdyndiff::EnergyPrescribedPositions::Params());
	auto bc2 = simulation.deformables->prescribed_positions->add_inside_aabb(H1.point_set, { -hd, hd, 0.0 }, { 0.001, 0.001, 0.001 }, nexdyndiff::EnergyPrescribedPositions::Params());
...
void hanging_box_with_composite_material()
{
	...
	*/
	...
	const double d = 0.2;
	const double hd = d/2.0;
	auto [vertices, tets] = nexdyndiff::generate_tet_grid({ 0.0, 0.0, 0.0 }, { d, d, d }, { n, n, n });

	// Meshes
	auto [triangles, tri_tet_map] = nexdyndiff::find_surface(vertices, tets);
	auto tri_vertices = nexdyndiff::gather(vertices, tri_tet_map);
	auto triangles_in_tet_connectivity = nexdyndiff::apply_map(triangles, tri_tet_map);
	auto [sharp_edges, edge_tri_map] = nexdyndiff::find_sharp_edges(tri_vertices, triangles, 30.0);
	std::vector<int> edge_tet_map = nexdyndiff::gather(tri_tet_map, edge_tri_map);
	auto edges_in_tet_connectivity = nexdyndiff::apply_map(sharp_edges, edge_tet_map);

	// Node set
	const nexdyndiff::PointSetHandler nodeset = simulation.deformables->point_sets->add(vertices)
		.add_rotation(-90.0, Eigen::Vector3d::UnitX());

	// Energies
	auto inertia = simulation.deformables->lumped_inertia->add(nodeset, tets, 
		nexdyndiff::EnergyLumpedInertia::Params()
		.set_density(1000.0)
		.set_damping(0.5)
	);
	auto tet_strain = simulation.deformables->tet_strain->add(nodeset, tets, 
		nexdyndiff::EnergyTetStrain::Params()
		.set_youngs_modulus(1e3)
	);
	auto segment_strain = simulation.deformables->segment_strain->add(nodeset, edges_in_tet_connectivity,
		nexdyndiff::EnergySegmentStrain::Params()
	...
	auto triangle_strain = simulation.deformables->triangle_strain->add(nodeset, triangles_in_tet_connectivity,
		nexdyndiff::EnergyTriangleStrain::Params()
	...
	auto discrete_shells = simulation.deformables->discrete_shells->add(nodeset, triangles_in_tet_connectivity,
		nexdyndiff::EnergyDiscreteShells::Params()
		.set_stiffness(2e-3)
	...
	auto bc = simulation.deformables->prescribed_positions->add_inside_aabb(nodeset, { hd, hd, hd }, { 0.001, 0.001, 0.001 },
		nexdyndiff::EnergyPrescribedPositions::Params()
		.set_stiffness(1e7)
		.set_tolerance(1e-3)
	);
...
void deformable_and_rigid_collisions()
{
	...
	*/
	...
	simulation.interactions->contact->set_global_params(
		nexdyndiff::EnergyFrictionalContact::GlobalParams()
	...
	// Deformables
	nexdyndiff::Volume::Params rubber = nexdyndiff::Volume::Params::Soft_Rubber();
	...
	auto [V, C, floor] = simulation.presets->rigidbodies->add_box("floor", 1.0, { d3, d3, 0.05*d3 }, nexdyndiff::ContactParams().set_contact_thickness(0.001*d3));
...
void simple_grasp()
{
	...
	*/
	...
	auto [obj, obj_c] = [&]()
		{
		auto params = nexdyndiff::Volume::Params::Soft_Rubber();
	...
	simulation.add_time_event(2.0, 3.0, 
		[&](double t) 
		{ 
			const double gz = nexdyndiff::blend(0.0, gravity, 2.0, 3.0, t, nexdyndiff::BlendType::Linear);
...
void twisting_cloth()
{
	...
	*/
	...
	settings.execution.end_simulation_time = 5.0;
	settings.simulation.gravity = { 0.0, 0.0, 0.0 };
	settings.newton.residual = { nexdyndiff::ResidualType::Acceleration, 100.0 };
	nexdyndiff::Simulation simulation(settings);
	...
	double s = 0.5;
	int n = 50;
	nexdyndiff::Surface::Params material = nexdyndiff::Surface::Params::Cotton_Fabric();
	...
	// BC
	auto bc_params = nexdyndiff::EnergyPrescribedPositions::Params();
...
void magnetic_deformables()
{
	...
	*/
	...
	// Add objects
	std::vector<nexdyndiff::Volume::Handler> objs;
	{
		double size = 0.1;
		int n = 2;
		...
		auto material = nexdyndiff::Volume::Params::Soft_Rubber();
	...
	simulation.add_time_event(1.5, 3.0,
		[&](double t) {
			double height = nexdyndiff::blend(magnet_height, 0.5, 1.5, 3.0, t, nexdyndiff::BlendType::Linear);
			magnet_fix.set_transformation({ 0.0, 0.0, height }, Eigen::Matrix3d::Identity());
		}
	);
	simulation.add_time_event(4.5, 6.0,
		[&](double t) {
			double height = nexdyndiff::blend(0.5, magnet_height, 4.5, 6.0, t, nexdyndiff::BlendType::Linear);
			magnet_fix.set_transformation({ 0.0, 0.0, height }, Eigen::Matrix3d::Identity());
		}
	);
...
void meshes_from_file()
{
	...
	*/
	...
	auto tet_mesh = nexdyndiff::load_vtk<4>(MODELS_PATH + "/armadillo_tet.vtk");  // Must be VTK 4.2

	// Create volumetric deformable object from tet mesh
	//// Material
	nexdyndiff::Volume::Params vol_params = nexdyndiff::Volume::Params::Soft_Rubber();
	...
	auto bc_tet = simulation.deformables->prescribed_positions->add_inside_aabb(tet_obj.point_set, { -0.94, 0.33, 0.91 }, { 0.22, 0.22, 0.22 }, 
		nexdyndiff::EnergyPrescribedPositions::Params().set_stiffness(1e7));
		
	// Load triangle mesh from VTK file
	auto tri_mesh = nexdyndiff::load_vtk<3>(MODELS_PATH + "/armadillo_tri.vtk");  // Must be VTK 4.2
	
	// Create surface deformable object from triangle mesh
	//// Material
	nexdyndiff::Surface::Params surf_params = nexdyndiff::Surface::Params::Cotton_Fabric();
