...
void template_sim(std::string name, std::function<void(nexdyndiff::Simulation& simulation, nexdyndiff::RigidBodyHandler& box0)> callback)
{
	...
	settings.output.console_verbosity = nexdyndiff::ConsoleVerbosity::Frames;
...
void rb_constraints_point()
{
	template_sim("ball_joint", 
		[](nexdyndiff::Simulation& simulation, nexdyndiff::RigidBodyHandler& box0) 
		{
			auto prev = box0;
			const int N = 10;
			for (int i = 1; i < N; i++) {
