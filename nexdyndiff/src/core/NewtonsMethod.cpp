...
nexdyndiff::core::NewtonState nexdyndiff::core::NewtonsMethod::solve(const double& dt, symx::GlobalEnergy& global_energy, Callbacks& callbacks, const Settings& settings, Console& console, Logger& logger)
{
...
void nexdyndiff::core::NewtonsMethod::_run_before_evaluation()
{
...
void nexdyndiff::core::NewtonsMethod::_run_after_evaluation()
{
...
symx::Assembled nexdyndiff::core::NewtonsMethod::_evaluate_E_grad_hess()
{
...
symx::Assembled nexdyndiff::core::NewtonsMethod::_evaluate_E_grad()
{
	this->_run_before_evaluation();
...
symx::Assembled nexdyndiff::core::NewtonsMethod::_evaluate_E()
{
	this->_run_before_evaluation();
...
Eigen::VectorXd nexdyndiff::core::NewtonsMethod::_compute_residual(const Eigen::VectorXd& grad, double dt)
{
	/*
		Note:
	...
	*/
	if (this->settings->newton.residual.type == ResidualType::Force) {
		...
		if (dof_count != (int)this->residual.size()) {
			std::cout << "nexdyndiff error: NewtonsMethod::_compute_residual() found that not all dofs were used for ResidualType::Acceleration." << std::endl;
			exit(-1);
		}
		...
		std::cout << "nexdyndiff error: NewtonsMethod::_compute_residual() found an unknown residual type." << std::endl;
		exit(-1);
	}
}
double nexdyndiff::core::NewtonsMethod::_compute_acceleration_correction(double du, double dt)
{
	return du / dt;  // Degrees of freedom are velocities in nexdyndiff. Therefore the correction du is also a velocity.
}
double nexdyndiff::core::NewtonsMethod::_forcing_sequence(const Eigen::VectorXd& rhs)
{
...
bool nexdyndiff::core::NewtonsMethod::_solve_linear_system(Eigen::VectorXd& du, const symx::Assembled& assembled, double dt)
{
	// Right hand side
	...
	if (this->settings->newton.linear_system_solver == LinearSystemSolver::DirectLU) {
		...
		std::cout << "nexdyndiff error: NewtonsMethod::_solve_linear_system() found an unknown linear system solver." << std::endl;
		exit(-1);
	}
}
double nexdyndiff::core::NewtonsMethod::_inplace_max_step_in_search_direction(const Eigen::VectorXd& du)
{
...
double nexdyndiff::core::NewtonsMethod::_inplace_backtracking_line_search(const Eigen::VectorXd& du, double E0, double E, double step_valid_configuration, double du_dot_grad)
{
