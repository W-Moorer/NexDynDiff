...
using namespace nexdyndiff::core;

NexDynDiff::NexDynDiff(const Settings& settings)
	: settings(settings)
{
	// Set
	...
	if (settings.output.codegen_directory == "") {
		std::cout << "NexDynDiff::setup() error: Settings.output.codegen_directory must be set" << std::endl;
		exit(-1);
	}
	if (settings.output.output_directory == "") {
		std::cout << "NexDynDiff::setup() error: Settings.output.output_directory must be set" << std::endl;
		exit(-1);
	}
	if (settings.output.simulation_name == "") {
		std::cout << "NexDynDiff::setup() error: Settings.output.simulation_name must be set" << std::endl;
		exit(-1);
	}
	...
	if (error) {
		std::cout << "NexDynDiff::setup() error: Cannot create folder " << settings.output.codegen_directory << std::endl;
		exit(-1);
	}

    std::filesystem::create_directories(settings.output.output_directory, error);
	if (error) {
		std::cout << "NexDynDiff::setup() error: Cannot create folder " << settings.output.output_directory << std::endl;
		exit(-1);
	}
...
bool NexDynDiff::run(double duration, std::function<void()> callback)
{
...
std::string NexDynDiff::get_frame_path(std::string name) const
{
...
void nexdyndiff::core::NexDynDiff::print()
{
	// Info
	this->console.print("\nInfo\n", ConsoleVerbosity::Frames);
...
void NexDynDiff::_initialize()
{
	this->is_init = true;

	if (this->global_energy.get_total_n_dofs() == 0) {
		this->console.print("NexDynDiff::_initialize() error: No degrees of freedom. Exiting simulation.", ConsoleVerbosity::Frames);
		exit(-1);
	}
