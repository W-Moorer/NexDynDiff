...
nexdyndiff::EnergyLumpedInertia::EnergyLumpedInertia(nexdyndiff::core::NexDynDiff& nexdyndiff, const spPointDynamics dyn)
	: dyn(dyn)
{
	// Energy definition
	nexdyndiff.global_energy.add_energy("EnergyLumpedInertia", this->conn,
		[&](symx::Energy& energy, symx::Element& node)
		{
			...
			symx::Vector gravity = energy.make_vector(nexdyndiff.gravity);
	...
	nexdyndiff.callbacks.add_inv_mass_application(this->dyn->dof, 
		[&](double* begin, double* end)
		{
			const int n = (int)std::distance(begin, end);
			if (n != 3 * this->dyn->size()) {
				std::cout << "nexdyndiff error: EnergyLumpedInertia::inv_mass_application() found `begin` and `end` with different size than the set nodes." << std::endl;
				exit(-1);
			}
...
nexdyndiff::EnergyLumpedInertia::Handler nexdyndiff::EnergyLumpedInertia::add(const PointSetHandler& set, const std::vector<int>& points, const std::vector<double>& lumped_volume, const Params& params)
{
...
nexdyndiff::EnergyLumpedInertia::Handler nexdyndiff::EnergyLumpedInertia::add(const PointSetHandler& set, const std::vector<double>& lumped_volume, const Params& params)
{
	set.exit_if_not_valid("EnergyLumpedInertia::add");
...
nexdyndiff::EnergyLumpedInertia::Handler nexdyndiff::EnergyLumpedInertia::add(const PointSetHandler& set, const std::vector<std::array<int, 2>>& edges, const Params& params)
{
	set.exit_if_not_valid("EnergyLumpedInertia::add");
...
nexdyndiff::EnergyLumpedInertia::Handler nexdyndiff::EnergyLumpedInertia::add(const PointSetHandler& set, const std::vector<std::array<int, 3>>& triangles, const Params& params)
{
	set.exit_if_not_valid("EnergyLumpedInertia::add");
	std::vector<double> lumped_volume(set.size(), 0.0);
...
nexdyndiff::EnergyLumpedInertia::Handler nexdyndiff::EnergyLumpedInertia::add(const PointSetHandler& set, const std::vector<std::array<int, 4>>& tets, const Params& params)
{
	set.exit_if_not_valid("EnergyLumpedInertia::add");
	std::vector<double> lumped_volume(set.size(), 0.0);
...
nexdyndiff::EnergyLumpedInertia::Params nexdyndiff::EnergyLumpedInertia::get_params(const Handler& handler) const
{
...
void nexdyndiff::EnergyLumpedInertia::set_params(const Handler& handler, const Params& params)
{
...
double nexdyndiff::EnergyLumpedInertia::get_mass(const Handler& handler) const
{
