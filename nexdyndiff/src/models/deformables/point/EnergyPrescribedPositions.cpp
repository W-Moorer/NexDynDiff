...
nexdyndiff::EnergyPrescribedPositions::EnergyPrescribedPositions(core::NexDynDiff& nexdyndiff, spPointDynamics dyn)
	: dyn(dyn)
{
	// Callbacks
	...
	// Declare the energy
	nexdyndiff.global_energy.add_energy("EnergyPrescribedPositions", this->conn,
...
nexdyndiff::EnergyPrescribedPositions::Handler nexdyndiff::EnergyPrescribedPositions::add(const PointSetHandler& set, const std::vector<int>& points, const Params& params)
{
...
nexdyndiff::EnergyPrescribedPositions::Handler nexdyndiff::EnergyPrescribedPositions::add_inside_aabb(const PointSetHandler& set, const Eigen::Vector3d& aabb_center, const Eigen::Vector3d& aabb_dim, const Params& params)
{
...
nexdyndiff::EnergyPrescribedPositions::Handler nexdyndiff::EnergyPrescribedPositions::add_outside_aabb(const PointSetHandler& set, const Eigen::Vector3d& aabb_center, const Eigen::Vector3d& aabb_dim, const Params& params)
{
...
nexdyndiff::EnergyPrescribedPositions::Params nexdyndiff::EnergyPrescribedPositions::get_params(const Handler& handler) const
{
	...
	if (group < 0 || group >= (int)this->stiffness.size()) {
		std::cout << "nexdyndiff error: EnergyPrescribedPositions::get_params() found an invalid index." << std::endl;
		exit(-1);
	}
...
void nexdyndiff::EnergyPrescribedPositions::set_params(const Handler& handler, const Params& params)
{
	handler.exit_if_not_valid("EnergyPrescribedPositions::set_params");

	const int group = handler.get_idx();
	if (group < 0 || group >= (int)this->stiffness.size()) {
		std::cout << "nexdyndiff error: EnergyPrescribedPositions::set_params() found an invalid index." << std::endl;
		exit(-1);
	}
...
void nexdyndiff::EnergyPrescribedPositions::set_transformation(const Handler& handler, const Eigen::Vector3d& t, const Eigen::Matrix3d& R)
{
...
void nexdyndiff::EnergyPrescribedPositions::set_transformation(const Handler& handler, const Eigen::Vector3d& t, const double angle_deg, const Eigen::Vector3d& axis)
{
...
void nexdyndiff::EnergyPrescribedPositions::set_target_position(const Handler& handler, int prescribed_idx, const Eigen::Vector3d& t)
{
...
bool nexdyndiff::EnergyPrescribedPositions::_is_converged_state_valid(nexdyndiff::core::NexDynDiff& nexdyndiff)
{
	...
	if (!is_valid) {
		nexdyndiff.console.add_error_msg("Deformable prescribed position constraints are not within tolerance. Stiffness hardened.");
	}
