...
symx::Matrix nexdyndiff::triangle_jacobian(const std::vector<symx::Vector>& x)
{
	// Projection matrix
...
symx::Matrix nexdyndiff::tet_jacobian(const std::vector<symx::Vector>& x)
{
	return symx::Matrix(symx::gather({ x[1] - x[0], x[2] - x[0] , x[3] - x[0] }), { 3, 3 }).transpose();
}
std::array<symx::Scalar, 2> nexdyndiff::eigenvalues_sym_2x2(const symx::Matrix& A)
{
...
std::array<symx::Scalar, 3> nexdyndiff::eigenvalues_sym_3x3(const symx::Matrix& A)
{
	// https://hal.science/hal-01501221/document
	const symx::Scalar& a = A(0, 0);
	const symx::Scalar& b = A(1, 1);
