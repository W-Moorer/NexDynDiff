...
bool nexdyndiff::PointSetHandler::is_valid() const
{
	return this->dyn != nullptr;
}
void nexdyndiff::PointSetHandler::exit_if_not_valid(const std::string& where_) const
{
	if (!this->is_valid()) {
		std::cout << "nexdyndiff error: Invalid handler found in " << where_ << std::endl;
		exit(-1);
	}
}
std::string nexdyndiff::PointSetHandler::get_label() const
{
	return this->dyn->labels[this->get_idx()];
}
PointSetHandler& nexdyndiff::PointSetHandler::set_label(const std::string& label)
{
...
std::vector<int> nexdyndiff::PointSetHandler::all() const
{
