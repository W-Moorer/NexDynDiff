...
namespace nexdyndiff
{
	...
	public:
		...
		struct GlobalParams
		{
			NEXDYNDIFF_PARAM_POSITIVE(double, default_contact_thickness, -1.0)  // Default parameter -1.0 means no default is set

			// Recommended (Default values are very soft. Probably a good idea to set them manually.)
			NEXDYNDIFF_PARAM_NON_NEGATIVE(double, min_contact_stiffness, 1e3)
			NEXDYNDIFF_PARAM_NON_NEGATIVE(double, friction_stick_slide_threshold, 0.1)

			// Enable/Disable
			NEXDYNDIFF_PARAM_NO_VALIDATION(bool, collisions_enabled, true)
			NEXDYNDIFF_PARAM_NO_VALIDATION(bool, friction_enabled, true)
		...
		struct Params
		{
			...
			NEXDYNDIFF_PARAM_POSITIVE(double, contact_thickness, 0.0)  
		};
		struct Handler 
		{ 
		private:
	...
	public:
		/* Methods */
		EnergyFrictionalContact(core::NexDynDiff& nexdyndiff, const spPointDynamics dyn, const spRigidBodyDynamics rb);
	...
	private:
		...
		// SymX callbacks
		void _before_time_step__update_friction_contacts(core::NexDynDiff& nexdyndiff);
		void _before_energy_evaluation__update_contacts(core::NexDynDiff& nexdyndiff);
		bool _is_intermidiate_state_valid(core::NexDynDiff& nexdyndiff, bool is_initial_check);
		void _on_intermidiate_state_invalid(core::NexDynDiff& nexdyndiff);
		void _on_time_step_accepted(core::NexDynDiff& nexdyndiff);

		// SymX definitions
		void _energies_contact_deformables(core::NexDynDiff& nexdyndiff);
		void _energies_contact_rb(core::NexDynDiff& nexdyndiff);
		void _energies_contact_rb_deformables(core::NexDynDiff& nexdyndiff);

		void _energies_friction_deformables(core::NexDynDiff& nexdyndiff);
		void _energies_friction_rb(core::NexDynDiff& nexdyndiff);
		void _energies_friction_rb_deformables(core::NexDynDiff& nexdyndiff);
		...
		// SymX setters
		void _set_barrier_potential(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Scalar& d, const symx::Index& group_a, const symx::Index& group_b);
		void _set_edge_edge_mollified_barrier_potential(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Scalar& d, const std::vector<symx::Vector>& ea, const std::vector<symx::Vector>& eb, const std::vector<symx::Vector>& ea_rest, const std::vector<symx::Vector>& eb_rest, const symx::Index& group_a, const symx::Index& group_b);
		void _set_friction_potential(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Vector& v, const symx::Index& contact_idx, const FrictionContact& contact);
		void _set_friction_point_edge(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Vector& vp, const std::vector<symx::Vector>& ve, const symx::Index& contact_idx, FrictionPointEdge& data);
		void _set_friction_point_triangle(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Vector& vp, const std::vector<symx::Vector>& vt, const symx::Index& contact_idx, FrictionPointTriangle& data);
		void _set_friction_edge_edge(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const std::vector<symx::Vector>& vea, const std::vector<symx::Vector>& veb, const symx::Index& contact_idx, FrictionEdgeEdge& data);

		// SymX data-symbol getters
		//// Rigid bodies
		std::vector<symx::Vector> _get_rb_v1(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Index& rb_idx, const std::vector<symx::Index>& conn);
		std::vector<symx::Vector> _get_rb_x1(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Index& rb_idx, const std::vector<symx::Index>& conn);
		std::vector<symx::Vector> _get_rb_X(symx::Energy& energy, const std::vector<symx::Index>& conn);
		std::array<std::vector<symx::Vector>, 2> _get_rb_edge(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Index& rb_idx, const std::vector<symx::Index>& conn);
		std::array<std::vector<symx::Vector>, 3> _get_rb_edge_point(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Index& rb_idx, const std::vector<symx::Index>& conn);

		//// Deformables
		std::vector<symx::Vector> _get_d_v1(symx::Energy& energy, const std::vector<symx::Index>& conn);
		std::vector<symx::Vector> _get_d_x1(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const std::vector<symx::Index>& conn);
		std::vector<symx::Vector> _get_d_X(symx::Energy& energy, const std::vector<symx::Index>& conn);
		std::array<std::vector<symx::Vector>, 2> _get_d_edge(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const std::vector<symx::Index>& conn);
		std::array<std::vector<symx::Vector>, 3> _get_d_edge_point(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const std::vector<symx::Index>& conn);
