...
EnergyFrictionalContact::EnergyFrictionalContact(core::NexDynDiff& nexdyndiff, const spPointDynamics dyn, const spRigidBodyDynamics rb)
	: dyn(dyn), rb(rb)
{
	if (!nexdyndiff.settings.simulation.init_frictional_contact) { return; }
...
void nexdyndiff::EnergyFrictionalContact::set_contact_thickness(const Handler& obj, const double contact_thickness)
{
	obj.exit_if_not_valid("EnergyFrictionalContact::set_contact_thickness");
	if (contact_thickness <= 0.0) {
		std::cout << "nexdyndiff error: Contact thickness must be positive in EnergyFrictionalContact." << std::endl;
		exit(-1);
	}
	this->contact_thicknesses[obj.get_idx()] = contact_thickness;
}
void nexdyndiff::EnergyFrictionalContact::set_friction(const Handler& obj0, const Handler& obj1, const double coulombs_coefficient)
{
...
void nexdyndiff::EnergyFrictionalContact::disable_collision(const Handler& obj0, const Handler& obj1)
{
...
bool nexdyndiff::EnergyFrictionalContact::is_empty() const
{
...
/* ===================================  HELPERS  =================================== */
/* ================================================================================= */
std::array<int, 2> nexdyndiff::EnergyFrictionalContact::_get_pair_key(const Handler& obj0, const Handler& obj1)
{
	return { std::min(obj0.get_idx(), obj1.get_idx()), std::max(obj0.get_idx(), obj1.get_idx()) };
}
double nexdyndiff::EnergyFrictionalContact::_init_contact_thickness(double contact_thickness)
{
	...
	if (contact_thickness == 0.0) {
		if (this->global_params.default_contact_thickness > 0.0) {
			...
			std::cout << "nexdyndiff error: Undefined contact thickness found. Explicitly declare per-object contact thickness or set a global default." << std::endl;
			exit(-1);
		}
	}
...
void nexdyndiff::EnergyFrictionalContact::_add_rigid_body(const RigidBodyHandler& rb, const Handler& handler, const std::vector<Eigen::Vector3d>& vertices)
{
	// Bookkeeping
...
void nexdyndiff::EnergyFrictionalContact::_add_deformable(const PointSetHandler& point_set, const Handler& handler, const std::vector<int>& point_set_map)
{
...
void EnergyFrictionalContact::_update_vertices(core::NexDynDiff& nexdyndiff, const double dt)
{
...
const tmcd::ProximityResults& EnergyFrictionalContact::_run_proximity_detection(core::NexDynDiff& nexdyndiff, const double dt)
{
...
const tmcd::IntersectionResults& EnergyFrictionalContact::_run_intersection_detection(core::NexDynDiff& nexdyndiff, const double dt)
{
	this->_update_vertices(nexdyndiff, dt);
...
std::array<int, N> EnergyFrictionalContact::_local_to_ps_global_indices(int group, const std::array<int, N>& local)
{
	...
	if (ps == PhysicalSystem::Deformable) {
		...
		std::cout << "nexdyndiff error: Unknown physical system found in EnergyFrictionalContact._local_to_ps_global_indices()\n" << std::endl;
		exit(-1);
	}
...
/* ===================================  SYMX CALLBACKS  =================================== */
/* ======================================================================================== */
void EnergyFrictionalContact::_before_energy_evaluation__update_contacts(core::NexDynDiff& nexdyndiff)
{
	if (!this->global_params.collisions_enabled) { return; }
	if (this->is_empty()) { return; }
...
void EnergyFrictionalContact::_before_time_step__update_friction_contacts(core::NexDynDiff& nexdyndiff)
{
	if (!this->global_params.collisions_enabled) { return; }
...
bool EnergyFrictionalContact::_is_intermidiate_state_valid(core::NexDynDiff& nexdyndiff, bool is_initial_check)
{
	...
	const auto& intersections = this->_run_intersection_detection(nexdyndiff, nexdyndiff.dt);
	...
	if (collision_found && is_initial_check) {
		...
		nexdyndiff.console.print("nexdyndiff error: Initial collision detected between the following contact object pairs:\n", core::ConsoleVerbosity::Frames);
		for (const auto& pair : colliding_objects) {
			nexdyndiff.console.print(fmt::format("\t- {} and {}\n", pair[0], pair[1]), core::ConsoleVerbosity::Frames);
		}
	}
...
void nexdyndiff::EnergyFrictionalContact::_on_intermidiate_state_invalid(core::NexDynDiff& nexdyndiff)
{
	...
	nexdyndiff.console.add_error_msg(fmt::format("Penetration couldn't be avoided. Contact stiffness hardened from {:.1e} to {:.1e}.", old_stiffness, new_stiffness));
}
void nexdyndiff::EnergyFrictionalContact::_on_time_step_accepted(core::NexDynDiff& nexdyndiff)
{
...
/* ===================================  SYMX DEFINITIONS  =================================== */
/* ========================================================================================== */
void EnergyFrictionalContact::_energies_contact_deformables(core::NexDynDiff& nexdyndiff)
{
	// Point - Triangle
	//// Point - Point
	nexdyndiff.global_energy.add_energy(this->_get_contact_label("d_d", "pt_pp"), this->contacts_deformables.point_triangle.point_point,
		[&](symx::Energy& energy, symx::Element& conn)
		{
			...
			this->_set_barrier_potential(energy, nexdyndiff, d, conn["group_a"], conn["group_b"]);
		}
	);
	...
	nexdyndiff.global_energy.add_energy(this->_get_contact_label("d_d", "ee_pp"), this->contacts_deformables.edge_edge.point_point,
		[&](symx::Energy& energy, symx::Element& conn)
		{
			...
			symx::Scalar d = distance_point_point(p[0], q[0]);
			this->_set_edge_edge_mollified_barrier_potential(energy, nexdyndiff, d, ea, eb, ea_rest, eb_rest, conn["group_a"], conn["group_b"]);
		}
	);
...
void EnergyFrictionalContact::_energies_contact_rb(core::NexDynDiff& nexdyndiff)
{
	// Point - Triangle
	//// Point - Point
...
void EnergyFrictionalContact::_energies_contact_rb_deformables(core::NexDynDiff& nexdyndiff)
{
	// Point - Triangle
	//// RB -> D: Point - Point
...
void EnergyFrictionalContact::_energies_friction_deformables(core::NexDynDiff& nexdyndiff)
{
	// Point - Point
...
void EnergyFrictionalContact::_energies_friction_rb(core::NexDynDiff& nexdyndiff)
{
	// Point - Point
	...
	nexdyndiff.global_energy.add_energy(this->_get_friction_label("rb_rb", "ee"), this->friction_rb.edge_edge.conn,
		[&](symx::Energy& energy, symx::Element& conn)
		{
			...
			this->_set_friction_edge_edge(energy, nexdyndiff, vea, veb, conn["idx"], this->friction_rb.edge_edge.data);
		}
	);
}
void EnergyFrictionalContact::_energies_friction_rb_deformables(core::NexDynDiff& nexdyndiff)
{
	// Point - Point
...
/* ===================================  SYMX SETTERS  =================================== */
/* ====================================================================================== */
symx::Scalar nexdyndiff::EnergyFrictionalContact::_get_contact_distance(symx::Energy& energy, const symx::Index& group_a, const symx::Index& group_b)
{
...
double nexdyndiff::EnergyFrictionalContact::_get_contact_distance(int group_a, int group_b)
{
	double dhat_a = this->contact_thicknesses[group_a];
	double dhat_b = this->contact_thicknesses[group_b];
	return dhat_a + dhat_b;
}
void EnergyFrictionalContact::_set_barrier_potential(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Scalar& d, const symx::Index& group_a, const symx::Index& group_b)
{
...
void EnergyFrictionalContact::_set_edge_edge_mollified_barrier_potential(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Scalar& d, const std::vector<symx::Vector>& ea, const std::vector<symx::Vector>& eb, const std::vector<symx::Vector>& ea_rest, const std::vector<symx::Vector>& eb_rest, const symx::Index& group_a, const symx::Index& group_b)
{
	symx::Scalar k = energy.make_scalar(this->contact_stiffness);
	symx::Scalar dhat = this->_get_contact_distance(energy, group_a, group_b);
...
void EnergyFrictionalContact::_set_friction_potential(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Vector& v, const symx::Index& contact_idx, const FrictionContact& contact)
{
	...
	symx::Scalar dt = energy.make_scalar(nexdyndiff.dt);
	symx::Scalar E = this->_friction_potential(v, fn, mu, T, epsv, dt);
	energy.set(E);
}
void EnergyFrictionalContact::_set_friction_point_edge(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Vector& vp, const std::vector<symx::Vector>& ve, const symx::Index& contact_idx, FrictionPointEdge& data)
{
...
void EnergyFrictionalContact::_set_friction_point_triangle(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Vector& vp, const std::vector<symx::Vector>& vt, const symx::Index& contact_idx, FrictionPointTriangle& data)
{
	symx::Vector bary = energy.make_vector(data.bary, contact_idx);
...
void EnergyFrictionalContact::_set_friction_edge_edge(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const std::vector<symx::Vector>& vea, const std::vector<symx::Vector>& veb, const symx::Index& contact_idx, FrictionEdgeEdge& data)
{
	symx::Vector bary = energy.make_vector(data.bary, contact_idx);
...
/* =================================================================================================== */
std::vector<symx::Vector> EnergyFrictionalContact::_get_rb_v1(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Index& rb_idx, const std::vector<symx::Index>& conn)
{
	symx::Scalar dt = energy.make_scalar(nexdyndiff.dt);
...
std::vector<symx::Vector> EnergyFrictionalContact::_get_rb_x1(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Index& rb_idx, const std::vector<symx::Index>& conn)
{
	symx::Scalar dt = energy.make_scalar(nexdyndiff.dt);
	std::vector<symx::Vector> x_loc = energy.make_vectors(this->rigidbody_local_vertices.data, conn);
...
std::array<std::vector<symx::Vector>, 2> EnergyFrictionalContact::_get_rb_edge(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Index& rb_idx, const std::vector<symx::Index>& conn)
{
...
std::array<std::vector<symx::Vector>, 3> EnergyFrictionalContact::_get_rb_edge_point(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const symx::Index& rb_idx, const std::vector<symx::Index>& conn)
{
...
std::vector<symx::Vector> EnergyFrictionalContact::_get_d_x1(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const std::vector<symx::Index>& conn)
{
...
std::array<std::vector<symx::Vector>, 2> EnergyFrictionalContact::_get_d_edge(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const std::vector<symx::Index>& conn)
{
	// conn = { conn["ea0"], conn["ea1"] }
...
std::array<std::vector<symx::Vector>, 3> EnergyFrictionalContact::_get_d_edge_point(symx::Energy& energy, const core::NexDynDiff& nexdyndiff, const std::vector<symx::Index>& conn)
{
	// conn = { conn["ea0"], conn["ea1"], conn["p"] }
	std::vector<symx::Vector> ea = this->_get_d_x1(energy, nexdyndiff, { conn[0], conn[1] });
	std::vector<symx::Vector> ea_rest = this->_get_d_X(energy, { conn[0], conn[1] });
