...
namespace nexdyndiff
{
	...
	public:
		...
		struct Params 
		{
			NEXDYNDIFF_PARAM_STIFFNESS()
			NEXDYNDIFF_PARAM_TOLERANCE()
		};
		struct Handler { NEXDYNDIFF_COMMON_HANDLER_CONTENTS(EnergyAttachments, Params) };
		using MultiHandler = MultiEnergyHandler<Handler, Params>;

	public:
		/* Methods */
		EnergyAttachments(core::NexDynDiff& nexdyndiff, const spPointDynamics dyn, const spRigidBodyDynamics rb);
	...
	private:
		...
		/* Methods */
		bool _is_converged_state_valid(nexdyndiff::core::NexDynDiff& nexdyndiff);
	};
}
