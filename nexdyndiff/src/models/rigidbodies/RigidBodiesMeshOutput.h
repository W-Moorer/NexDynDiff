...
namespace nexdyndiff
{
	...
	public:
		/* Methods */
		RigidBodiesMeshOutput(core::NexDynDiff& nexdyndiff, spRigidBodyDynamics rb);
	...
	private:
		...
		/* Methods */
		void _write_frame(core::NexDynDiff& nexdyndiff);

		template<std::size_t N>
		void _write(core::NexDynDiff& nexdyndiff, const MeshOutputGroups& groups, const std::vector<Mesh<N>>& meshes, std::vector<std::array<int, N>>& conn_buffer);
	};
}
