...
namespace nexdyndiff
{
	...
	class RBCLinearVelocityHandler
	{
		...
		inline auto& set_max_force(double force) { 
			...
			if (force < 0.0) {
				std::cout << "nexdyndiff error: RBCLinearVelocityHandler.set_max_force() found a negative force" << std::endl;
				exit(-1);
			}
	...
	class RBCAngularVelocityHandler
	{
		...
		inline auto& set_max_torque(double torque) { 
			...
			if (torque < 0.0) { 
				std::cout << "nexdyndiff error: RBCAngularVelocityHandler.set_max_abs_torque() found a negative torque" << std::endl;
				exit(-1);
			}
