	...
	: rb(rb)
{
	...
	// Linear inertia
	nexdyndiff.global_energy.add_energy("EnergyRigidBodyInertia_Linear", this->conn,
	...
	// Angular inertia
	nexdyndiff.global_energy.add_energy("EnergyRigidBodyInertia_Angular", this->conn,
		[&](symx::Energy& energy, symx::Element& conn)
		{
	...
	nexdyndiff.callbacks.add_inv_mass_application(this->rb->dof_v,
		[&](double* begin, double* end)
		{
			const int n = (int)std::distance(begin, end);
			const int n_bodies = this->rb->get_n_bodies();
			if (n != 3 * n_bodies) {
				std::cout << "nexdyndiff error: EnergyRigidBodyInertia::inv_mass_application() found `begin` and `end` with different size than the set nodes." << std::endl;
				exit(-1);
			}
	...
	nexdyndiff.callbacks.add_inv_mass_application(this->rb->dof_w,
		[&](double* begin, double* end)
		{
			const int n = (int)std::distance(begin, end);
			const int n_bodies = this->rb->get_n_bodies();
...
void nexdyndiff::EnergyRigidBodyInertia::add(const int rb_idx, const double mass, const Eigen::Matrix3d& inertia_loc, const double linear_damping, const double angular_damping)
{
	if (rb_idx != (int)this->mass.size()) {
		std::cout << "nexdyndiff error: EnergyRigidBodyInertia::add() found non-consequtive rigid body added." << std::endl;
		exit(-1);
	}
...
void nexdyndiff::EnergyRigidBodyInertia::_before_time_step(nexdyndiff::core::NexDynDiff& nexdyndiff)
{
