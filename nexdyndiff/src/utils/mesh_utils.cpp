...
double nexdyndiff::deg2rad(const double deg)
{
	return 2.0 * M_PI * (deg / 360.0);
}
double nexdyndiff::rad2deg(const double rad)
{
	return rad * 180.0 / M_PI;
}

std::vector<nexdyndiff::Mesh<3>> nexdyndiff::load_obj(const std::string& path)
{
    std::vector<nexdyndiff::Mesh<3>> tri_meshes;
...
void nexdyndiff::write_VTK(const std::string& path, const std::vector<Eigen::Vector3d>& vertices, const std::vector<std::array<int, 4>>& tets)
{
...
void nexdyndiff::write_VTK(const std::string& path, const std::vector<Eigen::Vector3d>& vertices, const std::vector<std::array<int, 3>>& triangles, const bool generate_normals)
{
	vtkio::VTKFile vtk_file;
...
void nexdyndiff::write_VTK(const std::string& path, const std::vector<Eigen::Vector3d>& vertices, const std::vector<std::array<int, 2>>& edges)
{
	vtkio::VTKFile vtk_file;
...
void nexdyndiff::write_VTK(const std::string& path, const std::vector<Eigen::Vector3d>& vertices, const std::vector<std::array<int, 1>>& points)
{
	vtkio::VTKFile vtk_file;
...
Eigen::Vector3d nexdyndiff::triangle_normal(const Eigen::Vector3d& p0, const Eigen::Vector3d& p1, const Eigen::Vector3d& p2)
{
	return (p0 - p2).cross(p1 - p2).normalized();
}
double nexdyndiff::triangle_area(const Eigen::Vector3d& p0, const Eigen::Vector3d& p1, const Eigen::Vector3d& p2)
{
	return 0.5 * (p0 - p2).cross(p1 - p2).norm();
}
double nexdyndiff::unsigned_tetra_volume(const Eigen::Vector3d& p0, const Eigen::Vector3d& p1, const Eigen::Vector3d& p2, const Eigen::Vector3d& p3)
{
	return std::abs(signed_tetra_volume(p0, p1, p2, p3));
}
double nexdyndiff::signed_tetra_volume(const Eigen::Vector3d& p0, const Eigen::Vector3d& p1, const Eigen::Vector3d& p2, const Eigen::Vector3d& p3)
{
	return (1.0 / 6.0) * ((p1 - p0).cross(p2 - p0)).dot(p3 - p0);
}

void nexdyndiff::find_node_node_map_simplex(std::vector<std::vector<int>>& output, const int32_t* connectivity, const int32_t n_simplices, const int32_t n_nodes_per_simplex, const int32_t n_nodes)
{
...
void nexdyndiff::find_internal_angles(std::vector<std::array<int, 4>>& internal_angles, const std::vector<std::array<int, 3>>& triangles, const int n_nodes)
{
	/*
	...
	*/
	...
	if (issue_warning) {
		std::cout << "nexdyndiff warning: triangle mesh has edges with more than two incident triangles." << std::endl;
	}
}
void nexdyndiff::find_perimeter_edges(std::vector<std::array<int, 2>>& out_perimeter_edges, std::vector<int>& out_edge_to_triangle_node_map, const std::vector<std::array<int, 3>>& triangles, const int n_nodes)
{
...
std::tuple<std::vector<std::array<int, 2>>, std::vector<int>> nexdyndiff::find_perimeter_edges(const std::vector<std::array<int, 3>>& triangles, const int n_nodes)
{
	std::vector<std::array<int, 2>> edges;
...
void nexdyndiff::find_surface(std::vector<std::array<int, 3>>& out_triangles, std::vector<int>& out_triangle_to_tet_node_map, const std::vector<Eigen::Vector3d>& vertices, const std::vector<std::array<int, 4>>& tets)
{
	out_triangles.clear();
...
std::tuple<std::vector<std::array<int, 3>>, std::vector<int>> nexdyndiff::find_surface(const std::vector<Eigen::Vector3d>& vertices, const std::vector<std::array<int, 4>>& tets)
{
...
void nexdyndiff::clean_triangle_mesh(std::vector<Eigen::Vector3d>& out_vertices, std::vector<std::array<int, 3>>& out_triangles, const std::vector<Eigen::Vector3d>& vertices, const std::vector<std::array<int, 3>>& triangles, const double merge_by_distance)
{
	...
	if (merge_by_distance > 0.0) {
		...
		if (max_n_cells_double > (double)std::numeric_limits<int>::max()) {
			std::cout << "nexdyndiff error: clean_triangle_mesh merge_by_distance is too small." << std::endl;
			exit(-1);
		}
...
std::tuple<std::vector<Eigen::Vector3d>, std::vector<std::array<int, 3>>> nexdyndiff::clean_triangle_mesh(const std::vector<Eigen::Vector3d>& vertices, const std::vector<std::array<int, 3>>& triangles, const double merge_by_distance)
{
...
void nexdyndiff::find_sharp_edges(std::vector<std::array<int, 2>>& out_edges, std::vector<int>& out_old_to_new_map, const std::vector<Eigen::Vector3d>& vertices, const std::vector<std::array<int, 3>>& triangles, double angle_deg_threshold)
{
	std::vector<std::array<int, 2>> edges;
...
std::tuple<std::vector<std::array<int, 2>>, std::vector<int>> nexdyndiff::find_sharp_edges(const std::vector<Eigen::Vector3d>& vertices, const std::vector<std::array<int, 3>>& triangles, double angle_deg_threshold)
{
...
double nexdyndiff::total_volume(const std::vector<Eigen::Vector3d>& vertices, const std::vector<std::array<int, 4>>& tets)
{
	double volume = 0.0;
...
void nexdyndiff::compute_node_normals(std::vector<Eigen::Vector3d>& output, const std::vector<Eigen::Vector3d>& vertices, const std::vector<std::array<int, 3>>& triangles)
{
...
void nexdyndiff::move(std::vector<Eigen::Vector3d>& points, const Eigen::Vector3d& translation)
{
	for (Eigen::Vector3d& point : points) {
		point += translation;
	}
}
void nexdyndiff::rotate_deg(std::vector<Eigen::Vector3d>& points, const double angle, const Eigen::Vector3d& axis)
{
...
void nexdyndiff::rotate_deg(std::vector<Eigen::Vector3d>& points, const double angle, const Eigen::Vector3d& axis, const Eigen::Vector3d& pivot)
{
	move(points, -pivot);
	rotate_deg(points, angle, axis);
	move(points, pivot);
}
void nexdyndiff::scale(std::vector<Eigen::Vector3d>& points, const Eigen::Vector3d& scale)
{
	for (Eigen::Vector3d& point : points) {
		point = scale.cwiseProduct(point);
	}
}
void nexdyndiff::scale(std::vector<Eigen::Vector3d>& points, const double s)
{
	scale(points, { s, s, s });
}
void nexdyndiff::mirror(std::vector<Eigen::Vector3d>& points, const int dim, const double pivot)
{
	for (Eigen::Vector3d& point : points) {
...
Eigen::Vector3d nexdyndiff::rotate_deg(const Eigen::Vector3d& point, const Eigen::Matrix3d& R, const Eigen::Vector3d& pivot)
{
