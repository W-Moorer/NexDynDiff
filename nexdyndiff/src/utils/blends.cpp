...
double nexdyndiff::blend(double min, double max, double begin_time, double end_time, double current_time, BlendType blendType)
{
    ...
    if (dt < 0.0 || dt > duration) {
        std::cout << "nexdyndiff error: blend() got time out of range: " << current_time << std::endl;
        exit(-1);
    }
    ...
    case BlendType::EaseInOut:
        ...
        break;
    default:
        std::cerr << "nexdyndiff error: blend() got unknown blend type." << std::endl;
        exit(1);
    }
